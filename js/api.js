...
export async function setAttendanceStatus({ employee_id, status, location }){
  const shift_date = new Date().toISOString().slice(0,10);
  const payload = { employee_id, shift_date, status, method:'manual', location: location||null, recorded_by:(await supabase.auth.getUser()).data.user.id, check_in:null, check_out:null };
  const ins = await supabase.from('attendance_records').insert(payload).select('id').maybeSingle();
  if(ins.error){
    const { data: row } = await supabase.from('attendance_records').select('id').eq('employee_id', employee_id).eq('shift_date', shift_date).maybeSingle();
    if(row){
      const update = { status, method:'manual', location: location||null, check_in:null, check_out:null };
      const { error: updErr } = await supabase.from('attendance_records').update(update).eq('id', row.id);
      if(updErr) throw updErr;
    }else{ throw ins.error; }
  }
...
export async function listMyAttendanceRange({from, to}){
  ...
  if(from) q.gte('shift_date', from); if(to) q.lte('shift_date', to);
...
export async function listMyViolationsRange({from, to}){
  ...
  if(from) q.gte('issue_datetime', from+'T00:00:00Z'); if(to) q.lte('issue_datetime', to+'T23:59:59Z');
  const { data, error } = await q; if(error) throw error; return data||[];
}
